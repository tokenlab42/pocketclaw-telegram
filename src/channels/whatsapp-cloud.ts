...
registerChannelAdapter('whatsapp-cloud', {
  factory: () => {
    ...
    // @chat-adapter/whatsapp hardcodes adapter.name to 'whatsapp', the same
    // channelType the native Baileys adapter registers under. Override so the
    // two coexist in activeAdapters (keyed by channelType) during the migration.
