...
export function buildSystemPromptAddendum(assistantName?: string): string {
  ...
  if (chromaCollectionId) {
    sections.push([
      ...
      '  - The collection contains documents of two types: report indices (`type = "report_index"`) and individual articles (`type = "article"`).',
      '  - CRITICAL: Whenever the user asks you for news, updates, or newsletters, you MUST query this shared `news` collection first. Retrieve the latest document where `type = "report_index"`. Present its content directly to the user as-is without any modification, rewriting, or reformatting (do NOT change bullet marks, headers, or add any introductory or closing remarks). Do not output the full descriptions for all articles upfront; instruct the user to ask or reply with the article number if they want to read the full content of a specific news article. If they do ask for a specific article number, determine the target report date first: 1) Inspect the recent chat history to find the date of the news report currently being discussed (e.g. from the broadcast message "*MOH Media Report (6 Jul 2026)*"). 2) If the date cannot be determined from the chat history, query Chroma for all report indices (`type = "report_index"`), compare their `date` metadata fields, and select the most recent date. Once the date is resolved, query Chroma for the document with metadata `type = "article"`, matching the resolved date and requested article number. Present the retrieved article document text directly. You MUST extract the source URL from the metadata `link` field (or from the document body) and always append it at the very end of your response as a raw clickable URL, formatted exactly as "Link: <url>".',
